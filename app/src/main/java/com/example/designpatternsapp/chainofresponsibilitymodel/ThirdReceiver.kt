...
class ThirdReceiver: BroadcastReceiver() {
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        Log.e("${ThirdReceiver::class.java.simpleName}" , "接收到广播")
        intent?.let {
            val limit = it.getIntExtra("limit", -1001)
            if(limit == 10){
                val message = it.getStringExtra("msg")
                val b = getResultExtras(true).getString("new")
                context?.let {
                        context ->
                    Toast.makeText(
                        context,
